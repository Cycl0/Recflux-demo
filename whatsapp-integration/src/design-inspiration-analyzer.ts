...
export interface InspirationSite {
  url: string;
  category: 'direct' | 'gallery' | 'theory';
  theme?: string;
  priority: number;
}
...
export interface ConsolidatedInsights {
  ...
  screenshots: {
    ...
    byUrl: { [url: string]: string[] };
    analysisResults: { [url: string]: { confidence: number; success: boolean; } };
  };
}
...
export class DesignInspirationAnalyzer {
   ...
   */
  selectInspirationSites(theme: string): InspirationSite[] {
    const themeNormalized = theme.toLowerCase();
    
    // REFERENCE SITES (exactly 1 selected)
    const referenceSites: { [key: string]: InspirationSite } = {
    ...
    const searchableGalleries: { [key: string]: InspirationSite } = {
    ...
    const staticGalleries: { [key: string]: InspirationSite } = {
    ...
    const theoryByTheme: { [key: string]: InspirationSite } = {
    ...
    const selectedSites: InspirationSite[] = [];
    ...
    let gallerySite: InspirationSite;
   ...
   */
  private getCrawlConfig(category: string): CrawlConfig {
    switch (category) {
      case 'direct':
        return {
          maxPages: 6,
   ...
   */
  private async performTextualAnalysis(sites: InspirationSite[]): Promise<{ [url: string]: any }> {
    const textualResults: { [url: string]: any } = {};
   ...
   */
  private async performVisualAnalysis(sites: InspirationSite[], bypassCache: boolean = false): Promise<{ [url: string]: any }> {
    ...
    let screenshotsByUrl: { [url: string]: string[] } = {};
   ...
   */
  private getVisualAnalysisPrompt(category: string): string {
    const basePrompt = `
...
`;
  ...
  private consolidateInsights(
    ...
    const consolidated: ConsolidatedInsights = {
      colorPalettes: {
        primary: [],
        secondary: [],
        accent: []
   ...
   */
  async analyzeDesignInspiration(theme: string, bypassCache: boolean = false): Promise<{
    ...
    summary: {
      totalSites: number;
      successfulTextual: number;
      successfulVisual: number;
    ...
    const sendProgressUpdate = (step: string, progress: number) => {
    ...
    const result = {
      ...
      summary: {
        ...
        cached: false as boolean | string
      }
    };
