...
export interface CacheStats {
  totalKeys: number;
  crawlCacheKeys: number;
  visualCacheKeys: number;
  designCacheKeys: number;
  puppeteerCacheKeys: number;
  imageGenerationCacheKeys: number;
  memoryUsage: string;
  hitRate?: number;
  uptime: number;
}
...
export interface DesignCacheEntry {
  ...
  textualResults: { [url: string]: any };
  visualResults: { [url: string]: any };
  summary: any;
  timestamp: string;
  ttl: number;
}

export interface PuppeteerCacheEntry {
  theme: string;
  url: string;
  scrapedData: {
    videos: Array<{ src: string, type: string, thumbnail?: string, duration?: string }>;
    images: Array<{ src: string, alt?: string, dimensions?: string, type?: string }>;
    icons: Array<{ src: string, type: string, usage?: string, size?: string }>;
    vectors: Array<{ src: string, format: string, usage?: string }>;
    colors: Array<{ color: string, usage?: string, frequency?: number }>;
    fonts: Array<{ family: string, weights: string[], usage?: string }>;
    socialMedia: Array<{ platform: string, url: string, type: string }>;
    backgroundImages: Array<{ src: string, usage: string, type?: string }>;
  };
  metadata: {
    scrapedAt: string;
    userAgent: string;
...
export interface ImageGenerationCacheEntry {
  prompt: string;
  imageUrl: string;
  allImages: string[];
  metadata: {
...
class RedisCache {
  ...
  async getCrawlAnalysis(url: string, outputFormat?: string): Promise<CrawlCacheEntry | null> {
    if (!this.isConnected) {
  ...
  async setCrawlAnalysis(url: string, data: Omit<CrawlCacheEntry, 'url' | 'timestamp'>, outputFormat?: string): Promise<boolean> {
    if (!this.isConnected) {
  ...
  async getVisualAnalysisByUrl(url: string, analysisType?: string): Promise<VisualCacheEntry | null> {
    if (!this.isConnected) {
  ...
  async getVisualAnalysis(screenshotPath: string, analysisType?: string): Promise<VisualCacheEntry | null> {
    if (!this.isConnected) {
      this.logMessage('[REDIS_CACHE] Not connected, skipping cache check for visual analysis');
      return null;
    }
  ...
  async setVisualAnalysisByUrl(url: string, data: Omit<VisualCacheEntry, 'screenshotPath' | 'timestamp'>): Promise<boolean> {
    if (!this.isConnected) {
    ...
    try {
      ...
      const cacheEntry: VisualCacheEntry = {
        ...data,
  ...
  async setVisualAnalysis(screenshotPath: string, data: Omit<VisualCacheEntry, 'screenshotPath' | 'timestamp'>): Promise<boolean> {
    if (!this.isConnected) {
      this.logMessage('[REDIS_CACHE] Not connected, skipping cache set for visual analysis');
      return false;
    }
  ...
  async getDesignAnalysis(theme: string): Promise<DesignCacheEntry | null> {
    if (!this.isConnected) {
  ...
  async setDesignAnalysis(theme: string, data: Omit<DesignCacheEntry, 'theme' | 'timestamp'>): Promise<boolean> {
    if (!this.isConnected) {
    ...
    try {
      const key = this.getDesignKey(theme);
      const cacheEntry: DesignCacheEntry = {
        ...data,
        theme,
        timestamp: new Date().toISOString(),
      };
  ...
  async getPuppeteerResults(theme: string, url: string): Promise<PuppeteerCacheEntry | null> {
    if (!this.isConnected) {
  ...
  async setPuppeteerResults(theme: string, url: string, data: Omit<PuppeteerCacheEntry, 'theme' | 'url' | 'timestamp'>): Promise<boolean> {
    if (!this.isConnected) {
    ...
    try {
      const key = this.getPuppeteerKey(theme, url);
      const cacheEntry: PuppeteerCacheEntry = {
        ...data,
        theme,
        url,
        timestamp: new Date().toISOString(),
  ...
  async getImageGenerationResults(prompt: string): Promise<ImageGenerationCacheEntry | null> {
    if (!this.isConnected) {
  ...
  async setImageGenerationResults(prompt: string, data: Omit<ImageGenerationCacheEntry, 'prompt' | 'timestamp'>): Promise<boolean> {
    if (!this.isConnected) {
    ...
    try {
      const key = this.getImageGenerationKey(prompt);
      const cacheEntry: ImageGenerationCacheEntry = {
        ...data,
        prompt,
        timestamp: new Date().toISOString(),
      };
  ...
  async clearCache(pattern: string = 'web_scraping:*'): Promise<number> {
    if (!this.isConnected) {
  ...
  async clearExpiredKeys(): Promise<number> {
