...
interface VideoMetadata {
    ...
    tags?: string[];
    type: string;
    searchQuery?: string; // Track which query found this video
}
...
export class VideoAnalyzer {
     ...
     */
    async analyzeAndSelectBestVideo(request: VideoAnalysisRequest): Promise<VideoAnalysisResult> {
        ...
        try {
            ...
            if (!request.bypassCache) {
                ...
                try {
                    ...
                    const cachedResult = await (cache as any).client?.get(cacheKey);
                    if (cachedResult) {
            ...
            if (!request.bypassCache) {
                try {
                    ...
                    await (cache as any).client?.setEx(cacheKey, ttl, JSON.stringify(result));
     ...
     */
    private createVideoAnalysisPrompt(request: VideoAnalysisRequest): string {
        const themeColorProfiles: Record<string, { colors: string[], mood: string, style: string }> = {
...
`;
     ...
     */
    private async analyzeThumbnails(videos: VideoMetadata[], prompt: string): Promise<string> {
        try {
     ...
     */
    private processAnalysisResults(videos: VideoMetadata[], analysisResponse: string, theme: string): VideoAnalysisResult {
        try {
            ...
            const alternatives = videos
                .map((video, index) => {
                    const scoreData = analysisData.scores?.find((s: any) => s.index === index);
                    return {
                        video,
     ...
     */
    private fallbackAnalysis(videos: VideoMetadata[], response: string): any {
