...
async function takeScreenshot(targetUrl: string, createGif: boolean = true): Promise<string> {
	...
	try {
		...
		await page.evaluate(() => new Promise(r => (globalThis as any).requestAnimationFrame(r)));
...
async function createAnimatedPreview(page: any): Promise<string> {
	...
	for (let frameIndex = 0; frameIndex < totalFrames; frameIndex++) {
		...
		await page.evaluate((progressValue: number) => {
			...
			const viewportHeight = (globalThis as any).window.innerHeight;
			...
			if (interactiveElements.length > 0) {
				...
				interactiveElements.forEach((el: any, idx: number) => {
...
async function sendWhatsappVideo(to: string, base64Video: string, caption?: string) {
	try {
		...
		if (dataPreview) console.error(`[WHATSAPP_API] Video send error body=${dataPreview}`);
		throw err;
	}
}
