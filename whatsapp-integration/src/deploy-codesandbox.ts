...
export async function deployToCodeSandbox(projectDir: string): Promise<{ deploymentUrl: string; previewUrl: string; editorUrl: string }> {
    console.log(`[CODESANDBOX_DEPLOY] Starting deployment from ${projectDir}`);
    
    const allFiles: Record<string, { content: any }> = {};
    ...
    async function collectSrcFiles(dir: string, prefix = 'src'): Promise<void> {
        try {
    ...
    for (const [filePath, fileData] of Object.entries(allFiles)) {
        if (typeof fileData.content === 'string' && fileData.content.includes('className=')) {
            ...
            console.log(`[CODESANDBOX_DEPLOY] Content preview: ${(fileData.content as string).substring(0, 200)}...`);
    ...
    if (hasTailwindClasses) {
        ...
        if (allFiles['public/index.html']) {
            let htmlContent = allFiles['public/index.html'].content as string;
        ...
        if (allFiles['src/index.css']) {
            const currentCSS = allFiles['src/index.css'].content as string;
