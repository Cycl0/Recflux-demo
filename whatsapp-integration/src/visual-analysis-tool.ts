...
function generateAnalysisPrompt(analysisType: string, targetFramework: string): string {
  const basePrompt = `
...
export function integrateVisualAnalysis() {
  return {
    ...
    async analyzeWebsiteScreenshots(urls: string[]) {
      const results = [];
...
async function takeWebsiteScreenshot(url: string): Promise<string> {
...
async function callVisualAnalysisTool(params: any): Promise<any> {
