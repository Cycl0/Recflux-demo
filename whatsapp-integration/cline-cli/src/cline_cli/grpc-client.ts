// Original file:
// https://github.com/cline/cline/blob/main/webview-ui/src/utils/vscode.ts
	...
	Object.values(service.methods).forEach((method) => {
		...
		client[method.name as keyof GrpcClientType<T>] = ((request: any) => {
			return new Promise((resolve, reject) => {
				...
				const handleResponse = (message: ExtensionMessage) => {
				...
				disposable = webview.onReceiveMessageFromExtension(handleResponse)
				...
				// Send the request
				webview.postMessageToExtension({
					type: "grpc_request",
					grpc_request: {
